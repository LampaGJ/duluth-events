...
/** Shared by every rendered page so `index.html` and `places.html` cannot drift apart visually. */
...
export function renderIndex(rows: Row[], sourceCount: number, builtAt: string, base: string): string {
  ...
  const venueNote =
    venueTotal > BY_VENUE_LIMIT
      ? `Showing the ${BY_VENUE_LIMIT} busiest of ${venueTotal} venue feeds — <a href="${base}/places.html">browse all venues</a>.`
      : "";
